...
export default function BrandsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const { data } = await brandsApi.getAll(page)
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-xl">
        <h1 className="text-h2 text-white">Marcas</h1>
        <button
          onClick={openCreate}
      ...
      <div className="mb-lg max-w-sm">
        <SearchBar value={search} onChange={setSearch} placeholder="Buscar marcas..." />
      </div>
      ...
      <div className="glass rounded-xl">
        {loading ? (
          ...
          <div className="p-xl text-center text-text-secondary">No hay marcas</div>
        ) : (
          <>
            <AdminTable headers={['Nombre', 'Productos', 'Estado', 'Acciones']}>
              {filtered.map((brand) => (
                <TableRow key={brand.id}>
                  <TableCell className="text-white font-medium">{brand.name}</TableCell>
                  <TableCell>{brand.productCount}</TableCell>
                  <TableCell><StatusBadge active={brand.active} /></TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <button onClick={() => openEdit(brand)} className="text-text-secondary hover:text-primary-400 transition-colors"><Pencil size={16} /></button>
                      <button onClick={() => setDeleting(brand)} className="text-text-secondary hover:text-error transition-colors"><Trash2 size={16} /></button>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <Modal isOpen={showForm} onClose={() => setShowForm(false)} title={editing ? 'Editar Marca' : 'Nueva Marca'}>
        <form onSubmit={handleSubmit} className="space-y-lg">
          ...
          <div>
            ...
            <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required maxLength={100} className="w-full bg-surface-secondary border border-border rounded-md py-2.5 px-4 text-white text-sm focus:outline-none focus:border-primary-500 transition-colors" />
          </div>
          <div>
            <label className="block text-text-secondary text-small mb-sm">URL Logo (opcional)</label>
            <input type="text" value={form.logo} onChange={(e) => setForm({ ...form, logo: e.target.value })} className="w-full bg-surface-secondary border border-border rounded-md py-2.5 px-4 text-white text-sm focus:outline-none focus:border-primary-500 transition-colors" />
          </div>
