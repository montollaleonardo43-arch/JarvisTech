...
export const POST = withApi(
  async (request: NextRequest, ctx: { params: Promise<{ promotionId: string }> }) => {
    const { promotionId } = await ctx.params
    const { file, altText, isPrimary } = await parseUpload(request)
    const result = await imageService.upload('promotion', Number(promotionId), file, altText, isPrimary)
